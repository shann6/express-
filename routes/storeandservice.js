...
router.get('/', async function (req, res) {
  ...
  const perpage = Number(req.query.perpage) || 51
  const limit = perpage
  const offset = (page - 1) * perpage

  const [rows] = await db.query(
    `SELECT * FROM storeandservice
 ${where} ${orderBy} LIMIT ${limit} OFFSET ${offset}`
  ...
  const [rows2] = await db.query(`SELECT COUNT(*) AS count FROM storeandservice
`)
  const { count } = rows2[0]
  // 計算總頁數
...
router.get('/:id', async function (req, res) {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM storeservice WHERE service_id = ?',
      [id]
    )
