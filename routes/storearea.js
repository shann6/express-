...
router.get('/', async function (req, res) {
  ...
  const [rows] = await db.query(
    `SELECT * FROM storearea ${where} ${orderBy} LIMIT ${limit} OFFSET ${offset}`
  )
  ...
  const [rows2] = await db.query(`SELECT COUNT(*) AS count FROM storearea`)
  const { count } = rows2[0]
  // 計算總頁數
...
router.get('/:id', async function (req, res) {
  ...
  try {
    const [rows] = await db.query('SELECT * FROM storearea WHERE area_id = ?', [
      id,
    ])
