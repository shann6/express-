...
router.get('/', async function (req, res) {
  ...
  //分類跟排序
  const sort = req.query.sort || 'reserve_id'
 ...
 ${where} ${orderBy} LIMIT ${limit} OFFSET ${offset}`
  ...
  const [rows2] = await db.query(`SELECT COUNT(*) AS count FROM storereserve
`)
  const { count } = rows2[0]
  // 計算總頁數
...
router.get('/:id', async function (req, res) {
  ...
  try {
    const [rows] = await db.query(
      'SELECT * FROM storereserve WHERE reserve_id = ?',
      [id]
    )
